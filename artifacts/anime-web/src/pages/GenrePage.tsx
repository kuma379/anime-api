import { useState } from "react";
import { useRoute } from "wouter";
import { useQuery } from "@tanstack/react-query";
import { getGenre } from "@/lib/api";
import AnimeCard from "@/components/AnimeCard";
import LoadingGrid from "@/components/LoadingGrid";

export default function GenrePage() {
  const [, params] = useRoute("/genre/:genre");
  const genre = params?.genre ?? "";
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["genre", genre, page],
    queryFn: () => getGenre(genre, page),
    enabled: !!genre,
  });

  const items = data?.data ?? data?.results ?? [];

  return (
    <main className="max-w-7xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-black text-white mb-6 capitalize">
        Genre: {genre?.replace(/-/g, " ")}
      </h1>

      {isLoading ? <LoadingGrid count={18} /> : (
        <>
          {items.length === 0 ? (
            <p className="text-gray-400 text-center py-20">Tidak ada data.</p>
          ) : (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 mb-6">
              {items.map((a: any) => (
                <AnimeCard key={a.id} id={a.id} title={a.title} image={a.image} type={a.type} />
              ))}
            </div>
          )}
          <div className="flex items-center justify-center gap-3 mt-6">
            <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="px-4 py-2 rounded-lg bg-white/10 text-white disabled:opacity-30 hover:bg-purple-600 transition-colors text-sm">← Prev</button>
            <span className="text-gray-400 text-sm">Hal {page}</span>
            <button disabled={items.length < 10} onClick={() => setPage(p => p + 1)} className="px-4 py-2 rounded-lg bg-white/10 text-white disabled:opacity-30 hover:bg-purple-600 transition-colors text-sm">Next →</button>
          </div>
        </>
      )}
    </main>
  );
}
